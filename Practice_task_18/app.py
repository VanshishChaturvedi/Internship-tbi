...
{% if no_admin_yet %}
<hr>
<h2>Register First Admin Account</h2>
<p style="color:blue;">
    No admin account exists yet in the system. This form is only shown because
    admin_users is currently empty — it disappears permanently once the first
    admin is created.
</p>
{% if admin_error_message %}<p style="color:red;">{{ admin_error_message }}</p>{% endif %}
<form action="/save_admin" method="POST">
    Name: <input type="text" name="admin_name" required><br><br>
    Email: <input type="email" name="admin_email" required><br><br>
    Password: <input type="password" name="admin_password" required minlength="6"><br><br>
    Re-enter Password: <input type="password" name="admin_confirm_password" required minlength="6"><br><br>
    Company (optional):
    ...
    <button type="submit">Register as First Admin (Superadmin)</button>
</form>
{% endif %}
...
# Admin Dashboard (separate from the employee dashboard above — admins
# have no company/department/feedback-submission fields on their own
# account, so they get their own simpler landing page with quick links
# into the admin-only tools instead)

HTML_ADMIN_DASHBOARD = """
<h2>Welcome, {{ admin_name }}!</h2>
<h3>Your Admin Account:</h3>
<ul>
    <li><strong>Admin ID:</strong> {{ admin_info[0] }}</li>
    <li><strong>Name:</strong> {{ admin_info[1] }}</li>
    <li><strong>Email:</strong> {{ admin_info[2] }}</li>
    <li><strong>Admin Role:</strong> {{ admin_info[3] }}</li>
    <li><strong>Company:</strong> {{ admin_info[4] if admin_info[4] else 'Not scoped to a company' }}</li>
    <li><strong>Account Created:</strong> {{ admin_info[5] }}</li>
</ul>
<br>
...
<h3>Quick Links</h3>
<a href="/admin/reports"><button>Reports</button></a>
<a href="/admin/users"><button>User Management</button></a>
<a href="/companies"><button>Company Management</button></a>
<a href="/questions"><button>Questions Management</button></a>
<a href="/feedbacks"><button>Feedback Management</button></a>
...
<a href="__EXPORT_URL__"><button>Export to CSV</button></a>
<br><br>
...
<p><strong>Date Submitted:</strong> {{ submission[0] }}</p>

{% if error_message %}<p style="color:red;">{{ error_message }}</p>{% endif %}

<form action="/my_feedback/{{ submission[1] }}/update" method="POST">
    {% for a in answers %}
    <div style="margin-bottom:20px;">
...
<a href="/export/employees?q={{ search_query }}&sort={{ sort_by }}"><button>Export to CSV</button></a>
<br><br>
<table border="1" cellpadding="8" style="border-collapse:collapse;">
    <tr>
        <th>Name</th>
...
<form action="/admin/users/{{ user[0] }}/update" method="POST">
    ...
    Status:
    <select name="status">
      <option value="active"   {{ 'selected' if user[5]=='active' }}>Active</option>
      <option value="inactive" {{ 'selected' if user[5]=='inactive' }}>Inactive</option>
    </select><br><br>
...
<div style="display:flex; flex-wrap:wrap; gap:12px; margin-bottom:24px;">
    ...
    <div style="border:1px solid #ccc; padding:14px 20px;">
        <strong>Total Feedback Submissions{{ stats.range_label }}</strong><br>
        <span style="font-size:28px;">{{ stats.total_submissions }}</span>
    </div>
    <div style="border:1px solid #ccc; padding:14px 20px;">
...
<hr>
<h3>Feedback Submissions by Company (within selected range)</h3>
<table border="1" cellpadding="8" style="border-collapse:collapse;">
    <tr><th>Company</th><th>Submissions</th></tr>
    {% for row in submissions_by_company %}
    <tr><td>{{ row[0] if row[0] else 'Unknown' }}</td><td>{{ row[1] }}</td></tr>
    {% else %}
    <tr><td colspan="2">No submissions in this range.</td></tr>
    {% endfor %}
</table>
...
def render_date_filter_block(action_url, filter_type, custom_start, custom_end, extra_hidden_fields=""):
    ...
    <form action="{action_url}" method="GET" style="margin-bottom:16px;">
        ...
        <select name="filter_type" id="filterTypeSelect" onchange="toggleCustomDates()">
            <option value="all"        {"selected" if filter_type == "all" else ""}>All Time</option>
            <option value="last_week"  {"selected" if filter_type == "last_week" else ""}>Last Week</option>
            <option value="last_month" {"selected" if filter_type == "last_month" else ""}>Last Month</option>
            <option value="custom"     {"selected" if filter_type == "custom" else ""}>Custom Range</option>
        </select>
        <span id="customDateFields" style="{"display:inline;" if filter_type == "custom" else "display:none;"}">
            Start: <input type="date" name="start_date" value="{custom_start}">
            End:   <input type="date" name="end_date" value="{custom_end}">
        </span>
        <button type="submit">Apply Filter</button>
        <a href="{action_url}"><button type="button">Clear</button></a>
    </form>
    <script>
...
def save_admin():
    ...
    cursor = conn.cursor()
    try:
        # Authoritative re-check — closes the bootstrap the instant one admin exists.
        cursor.execute("SELECT COUNT(*) FROM admin_users")
